...
const AssetPanel = (
  { title, assets, example, background }: { title: string, example: string, assets: object, background: string }
) => (
  <div class="shadow-lg">
    ...
    <div class="border-b border-t grid grid-cols-4 text-sm text-solid">
      {Object.entries(assets).map(([ name, path ]) => (
        <a class="border-r flex p-3 justify-center border-r transition hover:opacity-50" download={true} href={path}>
    ...
    <div class="my-10 pt-5 pb-10 container">
      <div class="mb-10 grid grid-cols-6 gap-4">
        <div class="col-span-4">
          The following are assets that represent the Solid brand. All assets are considered open-source
          contributions and should be used according to open standards and licensing rules.
        </div>
      ...
      <div class="grid grid-cols-3 gap-4">
        {assets.map((props) => <AssetPanel {...props} />)}
      </div>
    </div>
  </div>
);
