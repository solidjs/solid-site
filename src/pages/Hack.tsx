...
const Box: Component<BoxProps> = (props) => (
  <div
    class='md:grid md:grid-cols-12 md:space-x-6 bg-white/50 text-gray-700 max-w-3xl mx-auto mt-5 p-10 rounded-xl shadow-xl backdrop-blur-md'>
...
const Countdown: Component = () => {
  ...
  const countdownValues = createCountdown( (new Date()).getTime() < startDate.getTime() ? startDate : endDate);

  const CountdownBox = (props: { label: string, value: number }) => {
    const fixed = props.value >= 10 ? props.value.toString() : "0" + props.value.toString();

    return (
     <div class='flex flex-col items-center bg-gray-700 text-white p-2 rounded-md'>
  ...
  return (
    <div class='flex flex-row w-min gap-5'>
      <For each={Object.entries(countdownValues).slice(0, -1)}>{(entry) =>
...
const Hack: Component = () => {
  return (
    ...
    >
      <img
        class='w-6/6 md:w-5/6 mt-12 mb-4 g:w-4/6 xl:w-3/6 2xl:w-2/6 mx-auto'
        src='/img/hack.svg' />
      ...
      <div>
        <Box title='What'>
          ...
          corporate
          sponsors. We've got US$9,000 to give away to the best projects across
          three categories.
          It's free to enter, you've got three months to build your project, and
          winners will be selected by public vote.
        </Box>
        <Box title='When'>
          The event begins January 7th and you can start building then.
          The submissions portal will open in mid February, but you'll have
          until April 7th to submit.
          More details on categories and official rules and regulations
          will be announced via Solid News before the start date. Sign up below
          or join the <a href='https://discord.com/invite/solidjs'
                         class='underline'>Solid Discord</a> to make sure you're
          up-to-date!
        </Box>
        <Box title='Who'>
          The hackathon is open to anyone: experienced Solid users and newcomers
          alike. You'll use your GitHub account to verify
          your identity when you submit your project and when you cast your
          vote.
          The event is sponsored by corporate supporters of Solid and the
          open-source ecosystem.
          <div
            class='flex items-center space-y-5 flex-col md:space-y-0 md:flex-row mt-3 md:space-x-8 bg-white/20 p-4 px-7 rounded-xl'>
            <a
              target='_blank'
        ...
        <Box title='Why'>
          ...
          <p>
            ...
            world, and we're
            looking to push the boundaries of what a hackathon can be and what
            open-source projects can do. Ultimately, we want to give you an
            opportunity to give Solid a try.
          </p>
        </Box>

        <div
          class='bg-white/50 text-gray-700 max-w-3xl mx-auto mt-5 p-10 rounded-xl shadow-xl backdrop-blur-md'>
          <div class='mb-5'>
        ...
        <div
          class='bg-opacity-50 text-black max-w-3xl mx-auto mt-2 mb-20 p-10 rounded-xl'>
          <small>
            SolidHack is operated by volunteers and funded by corporate
            sponsors. The
            SolidHack Planning Committee is responsible for coordinating Rules
            and Regulations.
            Questions and concerns relating to the competition may be directed
            to community@solidjs.com. The Planning Committee maintains the right
            to enforce, adapt
