...
Solid 2.0 ships a new compiler toolchain written in Rust on top of [OXC](https://oxc.rs/), and `@solidjs/vite-plugin` **defaults to it automatically**. Upgrade the plugin and you're compiling Solid with native tooling. Zero configuration. Nothing to migrate. The Babel preset remains available as an option.
...
- **[Draft-first stores.](https://v2.solidjs.com/concepts/stores)** Setters take a draft you mutate; projections and derived stores (`createStore(fn)`, `createProjection`) replace the write-back patterns of 1.x.
...
Starting with those that directly support my work, [Sentry](https://sentry.io) my benevolent employer, and [Cursor](https://cursor.com/) whose credits made the impossible possible.
